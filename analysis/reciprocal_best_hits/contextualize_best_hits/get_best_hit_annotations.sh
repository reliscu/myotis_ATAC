...
for ea in *csv; do
  ...
  ## Get intersecting sequences into BED format for each species separately:
  ...
  ## Cross ref. intersecting sequences with genes:
  ...
  $bedtools2 intersect -wb -a $gff1 -b spec1.bed > 
    ../../contextualize_best_hits/results/${spec1}_${spec1}_vs_${spec2}_best_hits_gene_annotations.csv
  $bedtools2 intersect -wb -a $gff2 -b spec2.bed > 
    ../../contextualize_best_hits/results/${spec2}_${spec1}_vs_${spec2}_best_hits_gene_annotations.csv 
  
  ## Cross ref. intersecting sequences with TEs:

  gff1="/Users/rebecca/sudmant/analyses/myotis/data/genomes/bat_genomes/repeatMasker/${spec1}1_sorted.fas.out"
  gff2="/Users/rebecca/sudmant/analyses/myotis/data/genomes/bat_genomes/repeatMasker/${spec2}1_sorted.fas.out"

  ## Get TE annotations into BED format:
  if [ -f ../../contextualize_best_hits/resources/${spec1}_TEs.bed ]; then
    awk 'NR>3{gsub("SCAF", "SUPER", $5); print $5, $6, $7, $1, $2, $9, $11}' OFS='\t' $gff1 >
      ../../contextualize_best_hits/resources/${spec1}_TEs.bed
  fi
  if [ -f ../../contextualize_best_hits/resources/${spec1}_TEs.bed ]; then
    awk 'NR>3{gsub("SCAF", "SUPER", $5); print $5, $6, $7, $1, $2, $9, $11}' OFS='\t' $gff2 > 
  ...
  $bedtools2 intersect -wb -a ../../contextualize_best_hits/resources/${spec2}_TEs.bed $gff2 -b spec2.bed > 
