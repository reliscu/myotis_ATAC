...
for spec1 in ${species[@]}; do
    for spec2 in ${species[@]}; do
        if [ $spec1 != $spec2 ]; then
            echo $spec1 vs. $spec2
            $bedtools2 intersect -wo \
                -a ${spec1}_vs_${spec2}_${spec1}_positions_target.bed \
                -b ${spec1}_vs_${spec2}_${spec1}_positions_query.bed \
                > ${spec1}_vs_${spec2}_${spec1}_positions_intersect.bed
        fi
    done
done
