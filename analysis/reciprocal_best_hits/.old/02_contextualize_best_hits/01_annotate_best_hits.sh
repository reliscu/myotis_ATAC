`  cd /Users/rebecca/sudmant/analyses/myotis/analysis/reciprocal_best_hits/01_get_best_hits/results
  ...
  for ea in *csv; do
    ...
    $bedtools2 intersect -wb -a $gff1 -b spec1.bed > ../../02_contextualize_best_hits/resources/genes/${spec1}_vs_${spec2}_best_hits_genes.tsv
    $bedtools2 intersect -wb -a $gff2 -b spec2.bed > ../../02_contextualize_best_hits/resources/genes/${spec2}_vs_${spec1}_best_hits_genes.tsv
    ...
    if [ ! -f ../../02_contextualize_best_hits/resources/TEs/${spec1}_TEs.bed ]; then
      awk 'NR>3{gsub("SCAF", "SUPER", $5); print $5, $6, $7, $1, $2, $9, $11}' OFS='\t' $anno1 > ../../02_contextualize_best_hits/resources/TEs/${spec1}_TEs.bed
    fi
    if [ ! -f ../../02_contextualize_best_hits/resources/TEs/${spec2}_TEs.bed ]; then
      awk 'NR>3{gsub("SCAF", "SUPER", $5); print $5, $6, $7, $1, $2, $9, $11}' OFS='\t' $anno2 > ../../02_contextualize_best_hits/resources/TEs/${spec2}_TEs.bed
    fi
    $bedtools2 intersect -wb -a ../../02_contextualize_best_hits/resources/TEs/${spec1}_TEs.bed -b spec1.bed > ../../02_contextualize_best_hits/resources/TEs/${spec1}_vs_${spec2}_best_hits_TEs.tsv
    $bedtools2 intersect -wb -a ../../02_contextualize_best_hits/resources/TEs/${spec2}_TEs.bed -b spec2.bed > ../../02_contextualize_best_hits/resources/TEs/${spec2}_vs_${spec1}_best_hits_TEs.tsv
