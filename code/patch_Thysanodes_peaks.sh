...
awk -F"\t" '{temp=$1; for (i=2; i<=5; i++) {if (i<5) printf "%s\t", $i; if (i==5) printf "%s", temp} print ""}' OFS='\t' Consensus_Peaks_Annotations_Accross_All_Samples.csv > Consensus_Peaks_Annotations_Accross_All_Samples.bed 
